...
BEGIN
  ...
  SELECT
    ...
    'El cobro interno de ' || c.name || ' por $' || inv.amount || ' venció el ' || to_char(inv.due_date, 'DD/MM/YYYY') || '.' AS description,
    c.id AS company_id,
  ...
  SELECT
    ...
    'La compañía ' || c.name || ' tiene un cobro por vencer de $' || inv.amount || ' para el ' || to_char(inv.due_date, 'DD/MM/YYYY') || '.' AS description,
    c.id AS company_id,
    c.name::text AS company_name,
    inv.balance AS amount,
    inv.due_date AS due_date,
  ...
  SELECT
    ...
    'El plan ' || p.name || ' contratado por ' || c.name || ' tiene su próximo pago programado para el ' || to_char(sub.next_due_date, 'DD/MM/YYYY') || '.' AS description,
    c.id AS company_id,
    c.name::text AS company_name,
  ...
  SELECT
    ...
    'La cooperativa ' || c.name || ' se encuentra marcada como INACTIVA y sus usuarios tienen bloqueado el acceso.' AS description,
    c.id AS company_id,
    c.name::text AS company_name,
  ...
  FROM (
    SELECT
      ...
      'La compañía ' || c.name || ' tiene ' || count(m.id) || ' socios de un máximo de ' || p.max_members || ' (' || round((count(m.id)::numeric / p.max_members * 100)) || '%).' AS description,
      c.id AS company_id,
      c.name::text AS company_name
    FROM companies c
    ...
    GROUP BY c.id, c.name, p.max_members
    ...
    SELECT
      ...
      'La compañía ' || c.name || ' tiene ' || count(v.id) || ' unidades de un máximo de ' || p.max_vehicles || ' (' || round((count(v.id)::numeric / p.max_vehicles * 100)) || '%).' AS description,
      c.id AS company_id,
      c.name::text AS company_name
    FROM companies c
    JOIN company_subscriptions sub ON sub.company_id = c.id
    ...
    WHERE sub.status = 'active'
    GROUP BY c.id, c.name, p.max_vehicles
